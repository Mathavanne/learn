...
const ProductSec = () => {
  ...
  return (
    <div>
      ...
      <div className="text-center py-4">
        {Object.keys(dummyData).map((cat, idx) => (
          <button
            key={idx}
            onClick={() => handleCategoryClick(cat)}
            className="btn bg-blue-600 text-white m-2 px-4 py-2 rounded hover:bg-blue-800"
          >
            {cat}
          </button>
        ))}
        <button
          ...
          className="btn bg-gray-700 text-white m-2 px-4 py-2 rounded hover:bg-gray-900"
        >
          Show All
        </button>
  ...
  <span className='text-xs text-black tracking-widest font-medium title-font mb-1 '>Sub Catgories</span>
